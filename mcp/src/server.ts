 ...
 */
function validatePorts(ports: string | undefined | null): string {
  if (!ports) return "";
 ...
 */
function validateRemote(remote: string | undefined | null): string {
  if (!remote) return "";
 ...
 */
function run(executable: string, args: string[], ctx?: HandlerContext): Promise<string> {
...
const handler = async (name: string, args: any, ctx?: HandlerContext): Promise<ToolResult> => {
  ...
  switch (name) {
    case "list": {
      const cmdArgs: string[] = ["--console"];
      if (args?.ports) {
        const ports = validatePorts(args.ports);
        if (ports) cmdArgs.push("--ports", ports);
      }
      if (args?.docker) cmdArgs.push("--docker");
      if (args?.verbose) cmdArgs.push("--verbose");
      if (args?.remote) {
        const remote = validateRemote(args.remote);
        if (remote) cmdArgs.push("--remote", remote);
      }
      cmdArgs.push("--json");
    ...
    case "kill": {
      const ports = validatePorts(args?.ports);
      if (!ports) throw new Error("ports argument is required for kill");
      const cmdArgs: string[] = ["--kill-all", "--ports", ports];
      if (args?.remote) {
        const remote = validateRemote(args.remote);
        if (remote) cmdArgs.push("--remote", remote);
      }
    ...
    case "audit": {
      ...
      if (args?.suspiciousOnly) cmdArgs.push("--suspicious-only");
      if (args?.remote) {
        const remote = validateRemote(args.remote);
        if (remote) cmdArgs.push("--remote", remote);
      }
    ...
    case "cacheList": {
      const cmdArgs: string[] = ["cache", "--list"];
      if (args?.includeNpx) cmdArgs.push("--npx");
      if (args?.includeJsPm) cmdArgs.push("--js-pm");
      if (args?.includeHf) cmdArgs.push("--hf");
      if (args?.includeTorch) cmdArgs.push("--torch");
      if (args?.includeVercel) cmdArgs.push("--vercel");
      if (args?.includeCloudflare) cmdArgs.push("--cloudflare");
      if (args?.lang && args.lang !== "auto") {
        const lang = sanitizeInput(args.lang);
        if (lang) cmdArgs.push("--lang", lang);
      }
      if (args?.staleDays) {
    ...
    case "cacheClean": {
      const cmdArgs: string[] = ["cache", "--clean"];
      if (args?.includeNpx) cmdArgs.push("--npx");
      if (args?.includeJsPm) cmdArgs.push("--js-pm");
      if (args?.includeHf) cmdArgs.push("--hf");
      if (args?.includeTorch) cmdArgs.push("--torch");
      ...
      if (args?.safeDelete !== false) cmdArgs.push("--safe-delete");
